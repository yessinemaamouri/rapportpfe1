...
Cette section pose les bases conceptuelles indispensables à la compréhension du pipeline IA développé dans ce projet. Elle couvre successivement les notions d'intelligence artificielle, de vision par ordinateur et de détection d'objets, en insistant sur les mécanismes qui les relient et qui justifient leur usage conjoint dans un système KYC.
...
\begin{figure}[H]
  ...
  \caption{Relation hiérarchique entre Intelligence Artificielle, Machine Learning et Deep Learning}
...
C'est précisément cette capacité du Deep Learning à traiter des données visuelles complexes qui fonde la discipline de la vision par ordinateur, présentée dans la sous-section suivante.
...
\begin{figure}[H]
  ...
  \caption{Propagation avant et rétropropagation dans un réseau de neurones}
...
Ces réseaux de neurones constituent le socle de toutes les tâches de vision par ordinateur, dont la plus centrale pour ce projet est la détection d'objets, détaillée dans la sous-section suivante.
...
\begin{figure}[H]
  ...
  \caption{\textit{Illustration de la détection d'objets}}
  \label{fig:ch2-detection-objets}
\end{figure}

Pour comprendre la place de la détection d'objets dans l'écosystème de la vision par ordinateur, il est utile de la situer parmi les différents niveaux de complexité des tâches visuelles, puis d'examiner les deux grandes familles de méthodes de détection avant de présenter YOLO, le modèle retenu pour ce projet.
...
La détection d'objets, placée au centre de ce spectre, est la tâche retenue dans ce projet. Deux grandes familles architecturales permettent de la réaliser, chacune avec ses compromis en termes de vitesse et de précision.
...
C'est dans la famille One-Stage que s'inscrit l'algorithme YOLO, retenu pour ce projet en raison de son excellence en détection temps réel, présenté ci-après.
...
\begin{figure}[H]
  ...
  \caption{Logo officiel de l\x27architecture YOLO~\cite{ultralytics_docs}}
...
\begin{figure}[H]
  ...
  \caption{Principe de fonctionnement de YOLO~\cite{ultralytics_docs}}
  \label{fig:Principe de fonctionnement de YOLO }
\end{figure}

Comme illustré par la Figure~\ref{fig:Principe de fonctionnement de YOLO }, YOLO adopte une approche globale et unifiée :
...
Depuis l'introduction de YOLOv1, le modèle a connu de nombreuses itérations, chacune améliorant la précédente en termes de précision, de rapidité et d'efficacité~\cite{yolo11medium2024}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.90\textwidth]{images/image_chap2/TimelineCycle-removebg-preview.png}
  \caption{\textit{Chronologie de l'évolution des architectures YOLO (V1 à V11)}}
...
Le pipeline OCR se compose de cinq étapes séquentielles qui transforment une image brute en texte structuré exploitable~\cite{microsoft_ocr_pipeline} :

\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{images/image_chap2/optical-character-recognition.png}
  \caption{Pipeline OCR en cinq étapes~\cite{microsoft_ocr_pipeline}}
...
Ce processus permet de transformer une image contenant du texte en un format exploitable par un système informatique. Pour choisir le moteur le plus adapté au contexte KYC bilingue arabe/français, une analyse comparative des principales solutions open source a été réalisée.
...
\begin{table}[H]
  ...
  \begin{tabular}{|l|c|c|c|c|c|c|c|}
    ...
    \bottomrule
  \end{tabular}
  \caption{Benchmark des principaux moteurs OCR open source (mise à jour 2025)~\cite{ocr_benchmark_intuition,ocr_benchmark_acl2024,ocr_benchmark_arxiv2025,ocr_benchmark_koncile2026,ocr_benchmark_cisdem2025,ocr_benchmark_ijraset}}
...
Cette sous-section présente le modèle YOLO11m retenu pour la détection des champs textuels sur les documents d'identité. Toutes les valeurs de performance, paramètres et benchmarks cités proviennent de la documentation officielle Ultralytics~\cite{ultralytics_docs}.

\subsubsection{Présentation de YOLO11}

YOLO11 est la dernière génération de modèles de détection d'objets en temps réel développée par Ultralytics, publiée en septembre 2024~\cite{ultralytics_docs}. Il introduit plusieurs innovations architecturales majeures par rapport à ses prédécesseurs :
...
Le Tableau~\ref{tab:yolo11-variantes} présente les performances benchmarkées sur le dataset COCO (mAP50-95) pour chaque variante, en précisant le nombre de paramètres et la vitesse d'inférence CPU.
...
\begin{table}[H]
  ...
  \begin{tabular}{|l|c|c|c|c|l|}
    ...
    \bottomrule
  \end{tabular}
  \caption{Comparaison des variantes YOLO11 sur COCO val2017~\cite{ultralytics_docs}}
...
Le Tableau~\ref{tab:yolo11-vs-yolov8} compare YOLO11m à YOLOv8m, qui constitue la génération précédente la plus répandue en production.
...
\begin{table}[H]
  ...
  \begin{tabular}{|l|c|c|c|}
    ...
    \bottomrule
  \end{tabular}
  \caption{YOLO11m vs YOLOv8m : gains en précision et efficacité~\cite{ultralytics_docs}}
...
Cette sous-section présente le moteur OCR retenu pour l'extraction de texte sur les documents d'identité tunisiens. Elle couvre d'abord la plateforme PaddleOCR et son écosystème, puis détaille les apports spécifiques de la version PP-OCRv5 et les critères qui ont orienté ce choix pour le contexte bilingue du projet.

\subsubsection{PaddleOCR}

PaddleOCR est une boîte à outils OCR open source développée par Baidu dans le cadre de la plateforme de deep learning PaddlePaddle~\cite{paddlepaddle_hf}. Elle offre un ensemble complet de modèles pré-entraînés couvrant la détection de texte, la reconnaissance de caractères et l'analyse de mise en page pour plus de 80 langues. Légère, modulaire et compatible avec les déploiements embarqués comme les serveurs GPU, PaddleOCR s'est imposée comme l'une des solutions OCR open source les plus utilisées en production, notamment grâce à ses performances sur les scripts asiatiques et moyen-orientaux. La version PP-OCRv5, la plus récente, constitue une avancée significative dont les caractéristiques sont présentées ci-après.

\subsubsection{Présentation de PP-OCRv5}

PP-OCRv5 est la cinquième génération du pipeline OCR de PaddlePaddle (Baidu), publiée dans le cadre de PaddleOCR 3.0 en 2024~\cite{paddlepaddle_hf}. Il représente une évolution majeure conçue spécifiquement pour la reconnaissance de texte dans des environnements variés et multilingues, incluant des documents réels, du texte manuscrit et des images capturées en conditions non contrôlées.

L'architecture repose sur un pipeline à deux étages : détection des zones de texte (PP-OCRv5\_det), puis reconnaissance caractère par caractère (PP-OCRv5\_rec), disponible en variante mobile (optimisée CPU) et serveur (optimisée GPU). Le Tableau~\ref{tab:pp-ocr-evolution} compare ses performances aux versions précédentes.
...
\begin{table}[H]
  ...
  \begin{tabular}{|l|c|c|c|}
    ...
    \bottomrule
  \end{tabular}
  \caption{Évolution des performances PP-OCR à travers les versions~\cite{paddlepaddle_hf}}
  \label{tab:pp-ocr-evolution}
\end{table}

Ces résultats confirment la progression constante de PP-OCRv5 par rapport à ses prédécesseurs. Les quatre critères suivants expliquent pourquoi cette version répond précisément aux contraintes du projet KYC de la BTE.
...
\paragraph{Support natif de l'arabe}
...
\paragraph{Amélioration de précision documentée}
...
\paragraph{Performance face aux grands modèles de vision (VLM)}
...
\paragraph{Licence open source et facilité d'intégration}
